...
from pyfeature import Feature, Scenario, given, when, and_, then, flask_sqlalchemy_setup, clear_hooks, before_each_feature
from lite_task_flow import TaskFlowEngine
...
def test():
    ...
    def setup(feature):
        ...
        codernity_db = Database(mkdtemp())
        codernity_db.create()
        add_index(codernity_db)
        TaskFlowEngine(codernity_db)
        from lite_task_flow import register_task_cls
    ...
    with Feature(u"剩余重量异常", step_files=["lite_mms.test.at.steps.delivery"]):
        ...
        with Scenario(u'最简情况'):
            ...
            task_id = then(u'一个异常发货任务申请生成了', TaskFlowEngine.instance.db)
            when(u'批准该申请', task_id)
