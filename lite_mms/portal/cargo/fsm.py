...
class UnloadSessionFSM(StateMachine):
    ...
    def do_log(self, action, actor):
        #msg = u"用户(%s)对卸货会话%s执行了%s操作，卸货会话的状态从(%s)转变为(%s)" % (actor
        # .username, self.obj, cargo_const.desc_action(action),
        # self.last_state, self.current_state)
        msg = ""
        self.logger.info(msg, extra={"obj": self.obj.model, "actor": actor,
                                     "action": cargo_const.desc_action(action),
                                     "obj_pk": self.obj.id})
...
state_closed = StateClosed(fsm, {
    cargo_const.ACT_OPEN: (cargo_const.STATUS_LOADING, None)
})
