...
@committed
def _(step, group):
    return models.User(username=u"小明", password=md5("xm").hexdigest(),
                       groups=[group])
...
def _(step, plate, tare, delivery_session):
    ...
    with app.test_request_context():
        with app.test_client() as c:
            ...
            rv = c.get(url_for("delivery_ws.delivery_session",
                               id=delivery_session.id))
...
def _(step, delivery_session_dict, store_bill_list, order, sub_order):
    haystack = [sb["id"] for sb in delivery_session_dict["store_bills"][
        order.customer_order_number][str(sub_order.id)]]
...
def _(step, store_bills, delivery_session):
    ...
    with app.test_request_context():
        with app.test_client() as c:
            ...
            rv = c.post(
                url_for("delivery_ws.delivery_task", sid=delivery_session.id,
            ...
            return json_ret["id"], (set(json_ret["store_bill_id_list"]) - set(
                sb.id for sb in store_bills)).pop()
...
def _(step, new_sb_id):
    ...
    return StoreBillWrapper(
        models.StoreBill.query.filter(models.StoreBill.id == new_sb_id).one())
...
def _(step, sb_name, store_bill, delivery_session):
    assert store_bill.delivery_session.id == delivery_session.id
...
def _(step, sb_name, weight, store_bill):
    store_bill = models.StoreBill.query.filter(
        models.StoreBill.id == store_bill.id).one()
        ...
        with app.test_client() as c:
            rv = c.post(url_for("auth.login"),
                        data=dict(username=user.username,
                                  password='xm'))
            rv = c.post(
                url_for("delivery.delivery_task", id_=delivery_task_id),
                data={"weight": 30000}) # tare is 2000
...
def _(step, consignment, weight, user):
    ...
    with app.test_request_context():
        with app.test_client() as c:
            ...
            assert consignment.product_list[0].weight == weight
        ...
        with app.test_client() as c:
            rv = c.post(url_for("auth.login"),
                        data=dict(username=user.username,
                                  password='xm'))
            assert 302 == rv.status_code
            rv = c.post("/delivery/ajax/consignment", data={
                "task_id": consignment.delivery_session.delivery_task_list[0].id,
                "weight": weight})
