...
def department_list():
    ...
    departments = apis.manufacture.get_department_list()
    return json.dumps(
        [dict(name=t.name, id=t.id, team_id_list=[team.id for team in t.team_list]) for t in departments])


@manufacture_ws.route("/work-command/<int:id_>")
...
work command")
        ...
        team_id = IntegerField(u"team id")
        action = IntegerField(u"action", [validators.DataRequired(), validators.AnyOf(
            [ACT_ASSIGN, ACT_AFFIRM_RETRIEVAL, ACT_ADD_WEIGHT, ACT_END, ACT_CARRY_FORWARD, ACT_REFUSE,
             ACT_REFUSE_RETRIEVAL, ACT_AFFIRM_RETRIEVAL, ACT_QI, ACT_REFUSE_RETRIEVAL, ACT_RETRIVE_QI,
             ACT_QUICK_CARRY_FORWARD])])
...
@manufacture_ws.route("/quality-inspection-report", methods=["POST", "PUT", "DELETE"])
@webservice_call("json")
@permission_required_webservice(QualityInspectorPermission)
...
def off_duty():
    if not current_user.is_anonymous():
        ...
        for team in current_user.team_list:
            wc_list, total_cnt = manufacture.get_work_command_list(status_list=[STATUS_ENDING], team_id=team.id)
            for wc in wc_list:
                try:
                    wc.go(actor_id=current_user.id, action=ACT_CARRY_FORWARD)
                    cnt += 1
                except ValueError, e:
                    return unicode(e), 403
        return str(cnt)
